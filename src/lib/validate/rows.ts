...
import { ErrorLevelEnum, FieldType, ValidateRowType } from './validate.type';
...
export async function computeRows(
  ...
  const computedRows: ValidateRowType[] = await bluebird.map(
    parsedRows,
    (parsedRow: Record<string, string>, line: number) => {
...
type ReadValueType = {
  parsedValue: string | string[] | number | boolean | undefined;
